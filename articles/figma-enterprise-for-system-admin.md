...
ワークスペースアドミンはデフォルトのライブラリを設定したりパブリックリンクの設定を変更できます。
...
ただし、承認画面にはオブジェクト単位でしか情報が記載されていないので承認に必要な情報に不足感があります。通知欄の情報は本件に限らず最低限しかなく、もう少し詳細情報が欲しくなることがよくあります。メール通知には少しだけ詳しく記載されているので、通知機能については改善を期待しています。
